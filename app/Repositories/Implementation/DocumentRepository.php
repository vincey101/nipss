...
class DocumentRepository extends BaseRepository implements DocumentRepositoryInterface
{
    ...
    public function getDocuments($attributes)
    {
        ...
        if ($orderBy == 'categoryName') {
            ...
            $query = $query->orderBy('documents.deliveryDate', $direction);
        }


        if ($attributes->categoryId) {
            $query = $query->where(function ($query) use ($attributes) {
                $query->where('documents.categoryId', $attributes->categoryId)
        ...
        if ($attributes->clientId) {
            $query = $query->where(function ($query) use ($attributes) {
                $query->where('documents.clientId', $attributes->clientId);
            });
        }

        if ($attributes->statusId) {
            $query = $query->where(function ($query) use ($attributes) {
                $query->where('documents.statusId', $attributes->statusId);
            });
        }
    ...
    //     $query = $query->whereIn('documents.name', $documentIds);
    ...
    public function getDeepSearchDocuments($attributes)
    {
        // Get documents from full-text search index
        $results = $this->indexer->search($attributes->searchQuery, 10);
        $documentIds = isset($results['ids']) ? $results['ids'] : [];
        ...
        // Search by document name for all documents (both indexed and non-indexed)
        $searchQuery = strtolower($attributes->searchQuery);
        $query->where(function($q) use ($documentIds, $searchQuery) {
            if (!empty($documentIds)) {
                $q->whereIn('documents.id', $documentIds);  // Results from full-text search
            }
            $q->orWhere(DB::raw('LOWER(documents.name)'), 'LIKE', '%' . $searchQuery . '%');  // Direct name search
        });
    ...
    public function saveDocument($request, $path, $fileSize, $aiId)
    {
    ...
    if($request->documentRolePermissions){
         $documentRolePermissions = json_decode($request->documentRolePermissions);
                foreach ($documentRolePermissions as $docuemntrole) {
                    ...
                    foreach ($userIds as $userIdObject) {
                     ...
                     $chk_user = Users::where('id', $userIdObject->userId)->first();
        $email = $chk_user->email;
        $name = $chk_user->firstName .' '. $chk_user->lastName;
        $inputs =['email'=>$email,'name'=>$name];
    //     Mail::send('email',  $inputs, function ($message) use ($email,$name)  {
                        
    //         $message->from('support@nipss.com', $name ='Nipss');
    //         $message->subject("You Have a Memo Notification", $name);
    ...
    // });
        ...
        if($request->documentUserPermissions){
        ...
        $chk_user = Users::where('id', $docuemntUser->userId)->first();
        $email = $chk_user->email;
        $name = $chk_user->firstName .' '. $chk_user->lastName;
        $inputs =['email'=>$email,'name'=>$name];
    //      Mail::send('email',  $inputs, function ($message) use ($email,$name)  {
    ...
    // });
                ...
                $isTimeBound = false;
                  ...
                  // Find user with the AI-suggested position ID
                $userData = Users::where('positionId', $aiId)->get();
                ...
                foreach ($userData as  $user_Id) {
                    // Create permission for department head
                    DocumentUserPermissions::create([
                        'documentId' => $result->id,
                        'endDate' => $endDate  ?? '',
                        'isAllowDownload' => 1,
                    ...
                    // Create audit trail for auto-assignment
                    DocumentAuditTrails::create([
                        'documentId' => $result->id,
                        'createdDate' =>  Carbon::now(),
                        'operationName' => DocumentOperationEnum::Add_Permission->value,
                        'assignToUserId' => $user_Id->id
                    ]);
        ...
        $chk_user = Users::where('id', $user_Id->id)->first();
        $email = $chk_user->email;
        $name = $chk_user->firstName .' '. $chk_user->lastName;
        $inputs =['email'=>$email,'name'=>$name];
    //      Mail::send('email',  $inputs, function ($message) use ($email,$name)  {
    ...
    public function updateDocument($request, $id)
    {
        try {
            ...
            if ($model->statusId && $model->statusId != $oldStatusId) {
                $newStatus = DocumentStatus::find($model->statusId);
                if ($newStatus) {
                    $statusName = strtolower($newStatus->name);
                    if (strpos($statusName, 'complet') !== false || 
                        strpos($statusName, 'done') !== false ||
                        strpos($statusName, 'finish') !== false) {
                        
                        // Only set delivery date if it hasn't been set yet
